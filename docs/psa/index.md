# PSA Open Source — Blueprint de Produto

> Deep dive: mercado, concorrentes, features, arquitetura e roadmap para um PSA moderno, open source, AI-native.
> Tech base definida: **Go · React + TanStack · Zitadel · Restate.dev · Connect RPC + protobuf · shadcn/ui · Postgres · single binary**.
> Construído com o framework **Gofra** — co-desenvolvidos em paralelo.
...
Existe uma janela clara para um PSA **open source, moderno, rápido como Linear, extensível como VS Code, e AI-native desde o primeiro commit** — e você já escolheu uma stack que ataca diretamente as dores estruturais dos concorrentes.
...
5. Blueprint de arquitetura mapeado na sua stack
...
### Fase 0 — Setup do projeto (pré-requisito)

- Criar repositório Git independente para o PSA.
- Adicionar como `git submodule` em `gofra/psa/`.
- Estruturar `psa/` usando o layout gerado pelo Gofra (`gofra new` ou scaffold manual baseado no starter).
- Configurar `go.mod` do PSA com `replace` local apontando para `../` (Gofra) durante desenvolvimento.
- Primeiro commit do PSA: health check + `psa serve` + `psa migrate` funcionando, usando `runtime/` do Gofra.

**Semana 1–2**
- Definir nome, domínio, repo público, licença.
- Escrever o `README.md` e `VISION.md` com os 10 princípios.
- Setup Go + Vite + buf.build para protos dentro de `psa/`.
- Validar que o ciclo Gofra → PSA funciona: alterar algo no `runtime/`, ver refletido no PSA.
...
**Versão**: 1.1 — Adicionada estratégia de desenvolvimento dual (Gofra + PSA) e setup de git submodule
**Data**: 2026-04-18
